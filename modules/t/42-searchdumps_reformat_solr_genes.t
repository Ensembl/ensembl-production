...
"Interpro domain IPR025714 is found in 1 genes in Human and has 1 records from signature databases: PF13679"
...

	{
		...
		is( scalar( @{ $e->{synonyms} } ), 1 );
		is( $e->{domain_url},
			"Homo_sapiens/Location/Genome?ftype=Domain;id=IPR029063" );
		is( $e->{description},
"Interpro domain IPR029063 [test description] is found in 1 genes in Human and has 1 records from signature databases: 3.40.50.150"
		);
	}
};
subtest "Families reformat", sub {
	$formatter->reformat_gene_families( File::Spec->catfile(
														 $Bin, "genes_test.json"
										),
										$out_file,
										$genome, 'core' );
	...
	diag( Dumper($new_families) );
	{
		...
		is( $e->{domain_url},
"Homo_sapiens/Location/Gene?ftype=Family=PTHR12496;g=ENSG00000127720" );
		is( $e->{description},
"Ensembl protein family PTHR12496 [RRNAD1]: 1 gene / 1 proteins in Homo sapiens"
		);
	}
};
subtest "Gene trees reformat", sub {
	$formatter->reformat_gene_trees( File::Spec->catfile($Bin, "genes_test.json"
									 ),
									 $out_file,
									 $genome, 'core' );
	my $new_trees = decode_json( read_file($out_file) );
	{
		...
		is( $e->{domain_url},
			"Homo_sapiens/Gene/Compara_Tree?g=ENSG00000127720" );
		is( $e->{description},
"Gene METTL25 (ENSG00000127720} is a member of GeneTree ENSGT00530000063745" );
	}
};
done_testing;
