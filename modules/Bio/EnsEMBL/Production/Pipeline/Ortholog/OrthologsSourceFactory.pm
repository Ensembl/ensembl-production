...
package Bio::EnsEMBL::Production::Pipeline::Ortholog::OrthologsSourceFactory;
...
sub write_output {
    ...
    my $rapidrelease = $self->param('rapidrelease');
    ...
    foreach my $pair (keys $sp_config) {
        ...
        if ($rapidrelease) {
            $dir_name = $self->param('output_dir') . '/ensemblrapid';
        }
