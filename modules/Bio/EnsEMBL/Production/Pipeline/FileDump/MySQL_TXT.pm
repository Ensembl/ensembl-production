...
sub run {
    ...
    my $filter_tables = $self->param('filter_tables');
    ...
    my $tables = $self->get_tables($dba, $dbname, $filter_tables);
...
sub get_tables {
    my ($self, $dba, $database, $filter_tables) = @_;
    ...
    if (scalar @{$filter_tables} > 0) {
        $table_list_sql .= " AND TABLE_NAME in (".join(",", map {qq('$_')} @{$filter_tables}).")";
    }
