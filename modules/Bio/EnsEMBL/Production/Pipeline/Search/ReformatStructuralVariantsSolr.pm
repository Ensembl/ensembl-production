...
package Bio::EnsEMBL::Production::Pipeline::Search::ReformatStructuralVariantsSolr;
...
sub run {
	...
	if ( defined $dump_file ) {
		...
		my $dump_file_out = $sub_dir . '/' . $species . '_structuralvariants.json';
		$self->{logger}->info("Reformatting $dump_file into $dump_file_out");
		$reformatter->reformat_structural_variants(
