...
my $expected_files = {
    "embl"            => {
        "dir" => "{division}/embl/{species_dir}/", "expected" => [
        '{species_uc}.*.dat.gz',
        'README*',
        'CHECKSUMS*'
    ] },
    "genbank"         => {
        "dir" => "{division}/genbank/{species_dir}/", "expected" => [
        '{species_uc}.*.dat.gz',
        'README*',
        'CHECKSUMS*'
    ] },
    "gtf"             => {
        "dir" => "{division}/gtf/{species_dir}/", "expected" => [
        '{species_uc}.*.gtf.gz',
        'README*',
        'CHECKSUMS*'
    ] },
    "gff3"            => {
        "dir" => "{division}/gff3/{species_dir}/", "expected" => [
        '{species_uc}.*.gff3.gz',
        'README*',
        'CHECKSUMS*'
    ] },
    "vep"             => {
        "dir" => "{division}/variation/vep/{collection_dir}", "expected" => [
        '{species}_vep*.tar.gz',
        'CHECKSUMS*'
    ] },
    "json"            => {
        "dir" => "{division}/json/{species_dir}/", "expected" => [
        '{species}*.json',
        'CHECKSUMS*'
    ] },
    "fasta_pep"       => {
        "dir" => "{division}/fasta/{species_dir}/pep/", "expected" => [
        '{species_uc}.*.fa.gz',
        'README*',
        'CHECKSUMS*'
    ] },
    "fasta_cdna"      => {
        "dir" => "{division}/fasta/{species_dir}/cdna/", "expected" => [
        '{species_uc}.*.fa.gz',
        'README*',
        'CHECKSUMS*'
    ] },
    "fasta_cds"       => {
        "dir" => "{division}/fasta/{species_dir}/cds/", "expected" => [
        '{species_uc}.*.fa.gz',
        'README*',
        'CHECKSUMS*'
    ] },
    "fasta_dna"       => {
        "dir" => "{division}/fasta/{species_dir}/dna/", "expected" => [
        '{species_uc}.*.fa.gz',
        'README*',
        'CHECKSUMS*'
    ] },
    "fasta_dna_index" => {
        "dir" => "{division}/fasta/{species_dir}/dna_index/", "expected" => [
        '{species_uc}.*.fa.gz',
