...
my $tables = {
    master_external_db => {
        label    => 'db_name',
        cols     => [ qw/external_db_id db_name status priority db_display_name type secondary_db_name secondary_db_table description/ ],
        all_cols => [ qw/db_release external_db_id db_name status priority db_display_name type secondary_db_name secondary_db_table description/ ],
...
sub update_controlled_tables {
  ...
  if ($dbc->dbname =~ m/_cdna|core|otherfeatures|rnaseq|vega_/) {
    @tables = qw/attrib_type biotype external_db misc_set unmapped_reason analysis_description/;
  }
...
sub update_controlled_table {
  ...
  if ($table eq 'analysis_description') {
    ...
    if ($self->{update} == 1) {
      ...
      foreach my $row_id (keys %$mdata) {
        if (exists $data->{$row_id}) {
          if (join('', (map {$mdata->{$row_id}{$_} || ''} @{$tables->{$table}{cols}}))
              ne
              join('', (map {$data->{$row_id}{$_} || ''} @{$tables->{$table}{cols}}))) {
            $helper->execute_update(
...
sub update_analysis_description {
  ...
  throw "Could not determine species and type from $dbname" unless ($species && $type);

  if (!defined $logic_names || scalar(@$logic_names) == 0) {
    $logic_names = $dbc->sql_helper()->execute_simple(-SQL => 'SELECT logic_name FROM analysis JOIN analysis_description USING (analysis_id)');
  }
  ...
  my $analysis_types = $self->get_data_from_table(
    ...
    SELECT
        ...
        wda.data_value as web_data,
        ad.displayable as displayable
    FROM
        analysis_description ad
    LEFT OUTER JOIN web_data wd ON
        ad.web_data_id = wd.web_data_id
    LEFT JOIN web_data_element wda on
        wda.web_data_id = wd.web_data_id
    WHERE
        ad.logic_name = '$logic_name'/,
      'logic_name');
    ...
    if (scalar(keys %$logic_names) == 0 || exists $logic_names->{$logic_name}) {
      ...
      if (defined $analysis->{web_data}) {
        ...
        $dbc->sql_helper->execute_update(
            -SQL    => q/
            UPDATE  analysis_description ad
            JOIN    analysis a
              USING (analysis_id)
            SET   ad.description=?,
                  ad.display_label=?,
