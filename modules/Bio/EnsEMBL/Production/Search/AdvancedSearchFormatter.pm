...
package Bio::EnsEMBL::Production::Search::AdvancedSearchFormatter;
...
sub new {
    ...
    $self->{log} = get_logger();
    my ($tax_dba, $onto_dba, $retain_xrefs, $variation_dba, $meta_dba) =
        rearrange(
            [ qw/taxonomy_dba ontology_dba retain_xrefs variation_dba metadata_dba/ ],
            @args);
    ...
    $self->{meta_dba} = $meta_dba;
    if (!defined $self->{meta_dba}) {
        $self->log()->warn("No metadata DBA defined");
    }
...
sub remodel_genome {
    ...
    process_json_file(
        ...
        sub {
            ...
            print $out encode_json($new_gene);
...
sub remodel_gene {
    ...
    my $new_gene = $self->copy_hash(
        $gene,
        qw/id name description biotype analysis source seq_region_name start end strand coord_system homologues/
    );
    $self->collate_xrefs($gene, $new_gene);
    # process transcripts
    for my $transcript (@{$gene->{transcripts}}) {
        my $new_transcript = $self->copy_hash($transcript,
            qw/id name description biotype source analysis seq_region_name start end strand/);
    ...
    for my $homologue (@{$new_gene->{homologues}}) {
        $homologue->{genome_display} = $self->find_genome_name($homologue->{genome});
    }
    return $new_gene;
        ...
        if (defined $xref->{linkage_types} ||
            ...
            if (defined $xref->{associated_xrefs} &&
                ...
                for my $ass (@{$xref->{associated_xrefs}}) {
                    ...
                    if (defined $expanded_terms &&
                        scalar(@$expanded_terms) > 0) {
            ...
            -CALLBACK     => sub {
                ...
                if (defined $row->{consequence_types}) {
                    ...
                    for my $consequence_type (@{$row->{consequence_types}}) {
                        ...
                        $transcript->{consequence_types}->{$consequence_type} =
                            1;
                    }
                }
                return;
...
sub find_genome_name {
    ...
    if (!defined $name) {
        if (defined $self->{meta_dba}) {
            ...
            my $org = $self->{meta_dba}->get_GenomeOrganismInfoAdaptor()->fetch_by_name($genome);
