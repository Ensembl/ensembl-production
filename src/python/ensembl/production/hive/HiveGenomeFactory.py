...
import eHive
from ensembl.production.factories.metadata.genomeFactory import GenomeFetcher, GenomeQuery


class GenomeFactory(eHive.BaseRunnable):
  ...
  def run(self):
    ...
    genome_fetcher = GenomeFetcher(self.param_required("metadata_db_uri"))
    
    for genome in genome_fetcher.get_genomes(
      query_schema=GenomeQuery, #default query could be changed to specific query
