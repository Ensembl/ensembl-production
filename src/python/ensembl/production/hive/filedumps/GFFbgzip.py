...
class GFFbgzip(eHive.BaseRunnable):
    def run(self):
        ...
        os.system(f"sed -i '/###/d' {output_filename} && sed -i 's/##/0 ##/g' {output_filename} && sort -o {output_filename} -k1,1 -k4,4n -k5,5n -t$\'\\t\' {output_filename}  && sed -i 's/0 ##/##/g' {output_filename} && cat {output_filename} | bgzip -c > {bgzip_filename}")
