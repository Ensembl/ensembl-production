...
from ensembl.production.factories.metadata.model import Genome, GenomeDatabase, Organism, DataRelease, Division, Assembly
...
class Flow(str, Enum):
    all_species_flow = 1
    core_flow = 2
    # TODO: need to fetch the core db to get karyotype, expecying metadata-api will provide this info
    chromosome_flow = 3
    variation = 4
    compara_flow = 5
    regulation_flow = 6
...
class BaseFactory:
    ...
    def _base_filter(self, columns=None, **kwargs):
        ...
        if columns is None:
            columns = []
        columns = [Assembly.assembly_accession.label("assembly_accession"),
                   Assembly.assembly_name.label("assembly_name"), Division.name.label("division"),
                   Organism.name.label('species'), GenomeDatabase.dbname.label('dbname'),
                   GenomeDatabase.type.label('group')] if not len(columns) else columns
        ...
        if len(kwargs['ens_version']) == 0 and len(kwargs['ens_version']) == 0:
            raise ValueError(
                'ENS and EG versions missing,  provided them during initiation or export them as environment variables '
                ': ENS_VERSION=110; EG_VERSION=57')
...
class SpeciesFactory(DBFactory):
    ...
    def core_flow(self, **kwargs):
        ...
        columns = [Organism.name.label('species'), GenomeDatabase.dbname.label('dbname'),
                   GenomeDatabase.type.label('group')]
...
class Datafiles(SpeciesFactory):
    ...
    def _get_annotations_source_info(self, species: str, dbname: str, coredb_srv: str, meta_keys=None):
        """
        ...
        with db_connection.session_scope() as session:
            ...
            species_query = select(Meta.species_id).filter(Meta.meta_value == species).filter(
                Meta.meta_key == 'species.production_name')
            species_id = dict(session.execute(species_query).one())['species_id']

            core_query = select(Meta.meta_key, Meta.meta_value).filter(Meta.meta_key.in_(meta_keys)).filter(
    ...
    def set_annot_source(**kwargs):
        ...
        """
        kwargs['species.display_name'] = "_".join(kwargs.get('species.display_name', '').split(' ')[
                                                  0:2])  # Zootoca vivipara (Common lizard) - GCA_011800845.1
        kwargs['genebuild.initial_release_date'] = kwargs.get('genebuild.initial_release_date', '').replace('-',
                                                                                                            '_') if kwargs.get(
            'genebuild.initial_release_date', None) else ''
        kwargs['genebuild.last_geneset_update'] = kwargs.get('genebuild.last_geneset_update', '').replace('-',
                                                                                                          '_') if kwargs.get(
            'genebuild.initial_release_date', None) else ''
        kwargs['species.annotation_source'] = 'ensembl' if kwargs.get('species.annotation_source',
                                                                      '') == '' else kwargs.get(
            'species.annotation_source', '')
        return kwargs
    ...
    def get_datafiles(self, coredb_srv: str = None, base_path: str = None, **kwargs):
        ...
        try:
            ...
            for dataflow in self.core_flow(**kwargs):
                ...
                species_datafile = self.get_species_datafiles(species=dataflow_info['species'],
                                                              dbname=dataflow_info['dbname'],
