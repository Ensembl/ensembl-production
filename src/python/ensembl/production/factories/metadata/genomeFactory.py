...
class GenomeFetcher(DatasetFactory):
  ...
  def get_genomes(self, 
    ...
    if update_dataset_status: # kwargs.get('update_dataset_status'):
      try :
        for genome in result.data.get('genomeList', []):
          for dataset in genome.get('genomeDatasets', []):
            ...
            logger.info(f"Updated Dataset status for dataset uuid: {dataset.get('dataset')['datasetUuid']} to {status}  for genome {genome['genomeUuid']} ")
