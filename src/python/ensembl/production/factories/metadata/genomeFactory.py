...
class GenomeFilterInput(graphene.InputObjectType):
  genome_uuid = graphene.List(graphene.String, default_value=[])
...
class GenomeFetcher:
  ...
  def __init__(self, metadata_db_uri, query):
    self.schema = graphene.Schema(query=query)
    self.metadata_db = DBConnection(metadata_db_uri)
  ...
  def get_genomes(self,
    ...
    query = f"""
          {{
              genomeList(filters: {{
              genomeUuid: {self.list_to_string(genome_uuid)},
              ...
              datasetSourceType: {self.list_to_string(dataset_source_type)},
              organismGroupType: "{organism_group_type}",
              division: {self.list_to_string(division)},
              datasetType: {self.list_to_string(dataset_type)},
              antiDatasetType: {self.list_to_string(anti_dataset_type)},
              species : {self.list_to_string(species)}, 
              antiSpecies : {self.list_to_string(anti_species)},
              biosampleId : {self.list_to_string(biosample_id)}, 
              antiBiosampleId : {self.list_to_string(anti_biosample_id)},
              batchSize : {batch_size},
              runAll : {run_all},
              datasetStatus : {self.list_to_string(dataset_status)}
    ...
    with self.metadata_db.session_scope() as session:
      ...
      result = self.schema.execute(query, context_value={'session': session})
...
def main():
  ...
  with open(args.output, 'w') as json_output:
    genome_fetcher = GenomeFetcher(metadata_db_uri=args.metadata_db_uri, query=Query)
