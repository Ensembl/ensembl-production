...
def main():
    ...
    parser.add_argument('-d', '--organism_group', type=str, nargs='*', required=False, default=None,
                        help='versioned file name, ex: EnsemblVertbrates,EnsemblPlants')
    parser.add_argument('-p', '--organism_group_type', type=str, nargs='*', required=False, default=None,
                        help='organism group type, ex: Division')
    parser.add_argument('-u', '--unreleased_genomes', help='Fetch only unreleased genome and datasets',
                        action='store_true')
    parser.add_argument('-n', '--dataset_name', type=str, nargs='*', required=False, default=None,
                        help='ensembl dataset type to fetch unique genomes, ex: assembly, genebuild')
    parser.add_argument('-r', '--dataset_source', type=str, nargs='*', required=False, default=None,
                        help='ensembl dataset source, ex: homo_sapiens_core_111_38')
    parser.add_argument('-m', '--metadata_db_uri', type=str, required=True,
                        help='metadata db mysql uri, ex: mysql://ensro@localhost:3366/ensembl_genome_metadata')
    parser.add_argument('-t', '--taxonomy_db_uri', type=str, required=True,
