...
use Data::Dumper;
use DBI;
use Bio::EnsEMBL::Production::Utils::SchemaPatcher qw/list_patch_files find_missing_patches apply_patch/;
...
if(!defined $opts->{host} || !defined $opts->{basedir}) {
  croak "Usage: apply_patches.pl -host <host> -port <port> -user <user> -pass <pass> [-dbname <name>|-dbpattern <pattern>] -base_dir <ensembl dir>"
}
...
my $dbh = DBI->connect($dsn, $opts->{user}, $opts->{pass}) || croak "Could not connect to $dsn: $!";

my $available_databases = [];
if(!defined $opts->{pattern}) {
  if(defined $opts->{dbname}) {
    $opts->{pattern} = $opts->{dbname};
  } else {
    $opts->{pattern} = '.*';
  }
} 
