...
sub _aliases {
  ...
  if(! exists $self->{_aliases}) {
    my $dbc   = $self->_production_dbc();
    my %aliases = %{ $dbc->sql_helper()->execute_into_hash(
      -SQL => 'select alias, species_id from species_alias where is_current = 1'
    ) };
...
sub _aliases_to_add {
  ...
  foreach my $autogenerated (keys %{$species->{automatic_aliases}}) {
    #TODO We should be checking $aliases here not $species->{aliases}. Otherwise 
    #how do we know it's being used in another speices
    if(exists $aliases->{$autogenerated}) {
      if($aliases->{$autogenerated}) {
        $self->v('SKIPPING WARNING: %s is already used for species_id %s', $autogenerated, $aliases->{$autogenerated});
      }
    }
    else {
      push(@aliases_to_add, $autogenerated);
      $aliases->{$autogenerated} = $species->{id};
