...
sub _get_funcgen_DataFiles {
  ...
  foreach my $current_methylation_file (@$all_methylation_files) {

    my $data_file = Bio::EnsEMBL::DataFile->new(
        -adaptor      => $data_file_adaptor,
        -coord_system => $coord_system,
        -analysis     => $current_methylation_file->get_Analysis,
        -name         => $current_methylation_file->name,
        -url          => $current_methylation_file->file,
        -file_type    => $current_methylation_file->file_type,
    );
    push @funcgen_datafiles_found, $data_file;
  }
